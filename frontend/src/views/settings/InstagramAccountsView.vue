...
const editingAccount = ref<InstagramAccount | null>(null)
const testingAccountId = ref<string | null>(null)
const testResults = ref<Record<string, TestResult>>({})
const deleteDialogOpen = ref(false)
const accountToDelete = ref<InstagramAccount | null>(null)
...
async function saveAccount() {
  if (!formData.value.name.trim() || !formData.value.instagram_account_id.trim() || !formData.value.page_id.trim()) {
  ...
  try {
    ...
    if (editingAccount.value) {
      await api.put(`/instagram/accounts/${editingAccount.value.id}`, payload)
...
function openDeleteDialog(account: InstagramAccount) {
...
async function confirmDelete() {
  ...
  try {
    await api.delete(`/instagram/accounts/${accountToDelete.value.id}`)
...
async function testConnection(account: InstagramAccount) {
  testingAccountId.value = account.id
  try {
    const response = await api.post(`/instagram/accounts/${account.id}/test`)
...
const webhookUrl = window.location.origin + basePath + '/api/webhook/instagram'
</script>
  ...
  <div class="flex flex-col h-full">
    ...
    <header class="border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div class="flex h-16 items-center px-6">
        ...
        <Instagram class="h-5 w-5 mr-3 text-pink-500" />
        <div class="flex-1">
    ...
    <ScrollArea v-else class="flex-1">
      <div class="p-6 space-y-4 max-w-4xl mx-auto">
        <!-- Webhook URL Info -->
        <Card class="border-pink-200 dark:border-pink-800 bg-pink-50 dark:bg-pink-950">
          <CardContent class="p-4">
            <div class="flex items-start gap-3">
              <AlertCircle class="h-5 w-5 text-pink-600 dark:text-pink-400 mt-0.5" />
              <div class="flex-1">
                <h4 class="font-medium text-pink-900 dark:text-pink-100">Webhook Configuration</h4>
                <p class="text-sm text-pink-700 dark:text-pink-300 mt-1">
                  Configure this URL in your Meta Developer Console as the Instagram webhook callback URL:
                </p>
                <div class="flex items-center gap-2 mt-2">
                  <code class="px-2 py-1 bg-pink-100 dark:bg-pink-900 rounded text-sm font-mono">
                    {{ webhookUrl }}
                  </code>
        ...
        <Card class="border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-950">
          <CardContent class="p-4">
            <div class="flex items-start gap-3">
              ...
              <div class="flex-1">
                <h4 class="font-medium text-amber-900 dark:text-amber-100">Instagram Messaging Limitations</h4>
                <ul class="text-sm text-amber-700 dark:text-amber-300 mt-1 list-disc list-inside space-y-1">
                  <li>Instagram does not support message templates like WhatsApp</li>
                  <li>You can only reply to messages within a 24-hour window</li>
                  <li>Interactive buttons and lists are not available</li>
                  <li>Broadcast campaigns are not supported</li>
                </ul>
              </div>
            </div>
          </CardContent>
        ...
        <Card v-for="account in accounts" :key="account.id">
          <CardContent class="p-6">
            <div class="flex items-start justify-between">
              <div class="flex items-start gap-4">
                <div class="h-12 w-12 rounded-full bg-pink-100 dark:bg-pink-900 flex items-center justify-center flex-shrink-0">
                  <Instagram class="h-6 w-6 text-pink-600 dark:text-pink-400" />
                </div>
                <div class="min-w-0">
                  ...
                  <div v-if="testResults[account.id]" class="mt-2">
                    <div v-if="testResults[account.id].success" class="flex items-center gap-2 text-green-600 dark:text-green-400">
                      ...
                      <span v-if="testResults[account.id].username" class="text-sm text-muted-foreground">
                        - @{{ testResults[account.id].username }}
                      </span>
                    </div>
                  ...
                  <div class="mt-3 grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                    <div class="flex items-center gap-2">
                      <span class="text-muted-foreground">Instagram ID:</span>
                      <code class="text-xs bg-muted px-1 rounded">{{ account.instagram_account_id }}</code>
                      <Button variant="ghost" size="icon" class="h-6 w-6" @click="copyToClipboard(account.instagram_account_id, 'Instagram ID')">
                        <Copy class="h-3 w-3" />
                      </Button>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="text-muted-foreground">Page ID:</span>
                      <code class="text-xs bg-muted px-1 rounded">{{ account.page_id }}</code>
                      <Button variant="ghost" size="icon" class="h-6 w-6" @click="copyToClipboard(account.page_id, 'Page ID')">
                        <Copy class="h-3 w-3" />
                      </Button>
                    </div>
                    <div class="flex items-center gap-2">
        ...
        <Card v-if="accounts.length === 0">
          <CardContent class="py-12 text-center text-muted-foreground">
            <Instagram class="h-12 w-12 mx-auto mb-4 opacity-50 text-pink-500" />
            <p class="text-lg font-medium">No Instagram accounts connected</p>
            <p class="text-sm mb-4">Connect your Instagram Business account to receive and reply to Direct Messages.</p>
            <Button variant="outline" size="sm" @click="openCreateDialog">
              <Plus class="h-4 w-4 mr-2" />
              Add Account
            </Button>
        ...
        <Card>
          <CardContent class="p-6">
            ...
            <ol class="list-decimal list-inside space-y-3 text-sm text-muted-foreground">
              ...
              <li>Connect your Instagram Business or Creator account to a Facebook Page</li>
              <li>In Instagram &gt; Basic Display or Instagram Graph API, get your <strong>Instagram Business Account ID</strong></li>
              <li>
                Create a permanent access token with <code class="bg-muted px-1 rounded">instagram_basic</code> and <code class="bg-muted px-1 rounded">instagram_manage_messages</code> permissions
              </li>
              <li>Configure the webhook URL with the <code class="bg-muted px-1 rounded">messages</code> subscription field</li>
            </ol>
          </CardContent>
        </Card>
      </div>
    ...
    <Dialog v-model:open="isDialogOpen">
      <DialogContent class="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{{ editingAccount ? 'Edit' : 'Add' }} Instagram Account</DialogTitle>
          <DialogDescription>
        ...
        <div class="space-y-4 py-4">
          ...
          <div class="space-y-2">
            <Label for="instagram_account_id">Instagram Business Account ID <span class="text-destructive">*</span></Label>
            <Input
              id="instagram_account_id"
              v-model="formData.instagram_account_id"
              placeholder="e.g., 17841400000000000"
            />
            <p class="text-xs text-muted-foreground">
              Found in Meta Developer Console &gt; Instagram &gt; Basic Display
            </p>
          </div>

          <div class="space-y-2">
            <Label for="page_id">Facebook Page ID <span class="text-destructive">*</span></Label>
            <Input
              id="page_id"
          ...
          <div class="space-y-2">
            ...
            <Input
              ...
              type="password"
              placeholder="Page access token with Instagram permissions"
            />
            <p class="text-xs text-muted-foreground">
